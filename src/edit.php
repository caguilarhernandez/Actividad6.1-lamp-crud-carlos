...
if(isset($_POST['modifica'])) {
	...
	$surname = mysqli_real_escape_string($mysqli, $_POST['tipocomida']);
	$age = mysqli_real_escape_string($mysqli, $_POST['cuandocomer']);

	if(empty($nombre) || empty($tipocomida) || empty($cuandocomer))	{
		if(empty($nombre)) {
			echo "<font color='red'>Campo nombre vacío.</font><br/>";
		}

		if(empty($tipocomida)) {
			echo "<font color='red'>Campo tipo de comida vacío.</font><br/>";
		}

		if(empty($cuandocomer)) {
			echo "<font color='red'>Campo posicion para comer vacío.</font><br/>";
		}
	} //fin si
	else 
	{
//Prepara una sentencia SQL para su ejecución. En este caso una modificación de un registro de la BD.				
		$stmt = mysqli_prepare($mysqli, "UPDATE comidas SET nombre=?,tipocomida=?,cuandocomer=? WHERE id=?");
...
*/				
		mysqli_stmt_bind_param($stmt, "ssii", $nombre, $tipocomida, $cuandocomer, $id);
...
$stmt = mysqli_prepare($mysqli, "SELECT nombre, tipocomida, cuandocomer FROM comidas WHERE id=?");
//Enlaza variables como parámetros a una setencia preparada. 
mysqli_stmt_bind_param($stmt, "i", $id);
//Ejecuta una consulta preparada
mysqli_stmt_execute($stmt);
//Enlaza variables a una setencia preparada para el almacenamiento del resultado
mysqli_stmt_bind_result($stmt, $nombre, $tipocomida, $cuandocomer);
...
<head>
	...
	<title>Modificación comidas</title>
<!--	
...
Al hacer click en el botón Guardar, llama a esta misma página: edit.php-->
	<form action="edit.php" method="post">
		...
		<div>
			...
			<input type="text" name="tipocomida" id="tipocomida" value="<?php echo $surname;?>" required>
		</div>

		<div>
			<label for="cuandocomer">Momento en el que comer</label>
			<input type="text" name="cuandocomer" id="cuandocomer" value="<?php echo $age;?>" required>
		</div>
