...
//Obtiene los datos (name, surname y age) a partir del formulario de alta por el método POST (Se envía a través del body del HTTP Request. No aparece en la URL)
	...
	$tipocomida = mysqli_real_escape_string($mysqli, $_POST['tipocomida']);
	$cuandocomer = mysqli_real_escape_string($mysqli, $_POST['cuandocomer']);
	$paiscreacion = mysqli_real_escape_string($mysqli, $_POST['paiscreacion']);
	$paisauge = mysqli_real_escape_string($mysqli, $_POST['paisauge']);
...
//Comprueba si existen campos vacíos
	if(empty($nombre) || empty($tipocomida) || empty($cuandocomer) || empty($paiscreacion) || empty($paisauge)) 
	{
		if(empty($nombre)) {
		...
		if(empty($tipocomida)) {
			echo "<div>Campo tipo de la comida vacío</div>";
		}

		if(empty($cuandocomer)) {
			echo "<div>Campo cuando comer ese plato en una comida vacío.</div>";
		}
...
//Prepara una sentencia SQL para su ejecución. En este caso el alta de un registro de la BD.		
		$stmt = mysqli_prepare($mysqli, "INSERT INTO comidas (nombre,tipocomida,cuandocomer) VALUES(?,?,?)");
...
*/		
		mysqli_stmt_bind_param($stmt, "ssi", $nombre, $tipocomida, $cuandocomer, $paiscreacion, $paisauge);
...
?>
	...
	<footer>
    Creado por Carlos Aguilar Hernández &copy; 2024
  	</footer>
</div>
</body>
</html>
